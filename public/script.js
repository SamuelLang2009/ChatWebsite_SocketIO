...
socket.on("Recieve", (message,sender,reciever) => {
    if(counter%2 == 0){
        document.getElementById("messages").innerHTML += "<div class=\"unit\" style=\"background-color: white; padding:4px; outline: 1px solid white\"><label id = \"d"+counter+"\" class = \"lab\">"+sender+" to "+reciever+": </label><label id = \""+counter+"\" class = \"message\"><b>| "+message+"</b></label></div>";
    }
    else{
        document.getElementById("messages").innerHTML += "<div class=\"unit\" style=\"padding:4px; outline: 1px solid black\"><label id = \"d"+counter+"\" class = \"lab\">"+sender+" to "+reciever+": </label><label id = \""+counter+"\" class = \"message\"><b>| "+message+"</b></label></div>";
    }
    counter++;
...
socket.on("sendUsers", (users) => {
    ...
    for(const [key, value] of Object.entries(users)){
        if(value!=name){
            document.getElementById("dropdown").innerHTML += "<option value=\""+key+"\">"+value+"</option>";
        }
    }
})
...
function formatText(){
    ...
    for(let i = 0; i < labs.length; i++){
        document.getElementById(i).style.left = max_width-document.getElementById("d"+i).getBoundingClientRect().width + "px";
    }
}
